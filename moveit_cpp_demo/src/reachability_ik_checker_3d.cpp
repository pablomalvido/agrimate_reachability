...
public:
  ...
  void initialize()
  {
    ...
    for (int iter = 0; iter < 200; ++iter)
    {
        ...
        config_prune.y_offset_obstacle = 0.05; // Offset for the obstacle in the Y direction
        ...
        config_scan.y_offset_obstacle = 0.2;
        ...
        config_grasp.y_offset_obstacle = 0.05;
...
std::vector<double> computeRobotConfigFromTable(
  ...
  void addBoxObstacle_2(const geometry_msgs::msg::Pose& target_pose, double y_offset)
  {
    double dim_y = 0.05; // Width of the box in the Y direction
    moveit_msgs::msg::CollisionObject obj;
    obj.id = "temp_box_2";
    obj.header.frame_id = base_frame_;
    obj.pose.position = target_pose.position;
    obj.pose.position.x = 0.15;
    obj.pose.position.y -= (y_offset + dim_y/2.0); // Adjust Y position based on the offset and box width
    obj.pose.orientation.w = 1.0;
    ...
    shape_msgs::msg::SolidPrimitive primitive;
    primitive.type = primitive.BOX;
    primitive.dimensions = {1.0, dim_y, dim_z};
    ...
    for (int i = 0; i < total_poses; ++i)
    {
      ...
      if (found_ik)
      {
            ...
            plan_dist(gen))
        {
          ...
          addBoxObstacle_2(pose, config.y_offset_obstacle);
